...
public class KnnKfold {
	...
	public KnnKfold(ArrayList<Amostra> amostras, int n_amostras, int n_vizinhos, int n_particoes) {
		...
		// Particionar (K-FOLD) direto no construtor para não ter que armazenar o
		// ArrayList original
	...
	public void classificar() {
		...
		for (int particaoTeste = 0; particaoTeste < n_particoes; particaoTeste++) { // Pega uma partição como teste por
																					// vez
			...
			for (Amostra a_teste : particoes.get(particaoTeste)) { // E Para cada amostra de teste dessa partição
				...
				for (int particaoTreinamento = 0; particaoTreinamento < n_particoes; particaoTreinamento++) { // Em cada
																												// partição
					if (particaoTreinamento != particaoTeste) { // Que não for a partição de teste
						for (Amostra a_treino : particoes.get(particaoTreinamento)) { // E Para cada amostra de treino
							double dist = distEuclidiana(a_teste, a_treino); // Calcular a distância entre a amostra de
																				// teste
				...
				// Aqui já temos as distâncias de a_teste calculada para todas a_treinamento de
				// todas as partições
				...
				do {
					int cont = 0;
					int maior = 0;
					HashMap<Integer, Integer> n_vezes = new HashMap<>(); // Armazenar a classe e quantas vezes ela
																			// aparece entre os vizinhos
					ArrayList<Double> menores = new ArrayList<>(distancias.keySet()); // Pega as distâncias calculadas,
																						// ordenadas de forma crescente

					while (cont < n_vizinhos - ajuste) { // Até a quantidade de vizinhos informada (menos o ajuste, se
															// já houve empate)
						...
						for (Amostra v : distancias.get(dist)) { // Pega a(s) amostra(s) com aquela distância
							...
							// System.out.println("Vizinho " + v.toString() + " Distância " +
							// dist);//Impressão para conferência
							if (n_vezes.containsKey(v_classe)) { // E adiciona a classe desse vizinho para fins de
																	// contagem
								n_vezes.replace(v_classe, n_vezes.get(v_classe) + 1);
							} else {
								n_vezes.put(v_classe, 1);
							}
							if (n_vezes.get(v_classe) > maior) { // Vai atualizando a maior quantidade de vezes que uma
																	// mesma classe repete
					...
					for (int r : n_vezes.keySet()) {
						if (n_vezes.get(r) == maior) { // Confere cada classe e só trata a que repetiu mais vezes
							...
							if (!ok) {
								...
								ok = false; // Se achou uma segunda classe com o máximo de vezes que repetiu, para e
											// reinicia o processo com um vizinho a menos
								ajuste++;
								break;
							}
						}
				...
				// Parte 3: conferir resultado com a classe da amostra e atualizar a taxa de
				// acerto

				// System.out.print("Partição " + (particaoTeste + 1) + ", Amostra " +
				// a_teste.toString() + ", Resultado: " + resultado + " (");
